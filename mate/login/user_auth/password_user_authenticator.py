...
class PasswordUserAuthenticator(AbstractUserAuthenticator):
    ...
    def auth_user(self, username: str, login_type: str, secret, client_type: str, is_staff: bool, payload=None) -> bool:
        ...
        if number_results != 1:
            app.logger.warning("Got {} credential secrets for username {}".format(number_results, username))
        ...
        app.logger.info(db_secret)
        if payload is not None:
            app.logger.warning("PasswordUserAuthenticator received unsupported payload data, Expected was None")
